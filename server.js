...
app.get("/api/resources",auth,async(_req,res)=>{const{data,error}=await supabase.from("studynest_resources").select("id,title,subject,category,original_name,size,created_at,studynest_users(name)").order("created_at",{ascending:false});if(error)throw error;res.json(data.map(r=>({id:r.id,title:r.title,subject:r.subject,category:r.category,filename:r.original_name,size:r.size,createdAt:r.created_at,uploader:(Array.isArray(r.studynest_users)?r.studynest_users[0]?.name:r.studynest_users?.name)||"Student"})))});
app.post("/api/resources",auth,upload.single("file"),async(req,res)=>{if(!req.file)return res.status(400).json({error:"Valid file required"});const title=String(req.body.title||"").trim().slice(0,160),subject=String(req.body.subject||"").trim().slice(0,100),category=String(req.body.category||"Notes").slice(0,40);if(!title||!subject)return res.status(400).json({error:"Title and subject required"});const ext=path.extname(req.file.originalname).toLowerCase(),storagePath=`${crypto.randomUUID()}${ext}`;const{error:uploadError}=await supabase.storage.from("studynest-files").upload(storagePath,req.file.buffer,{contentType:req.file.mimetype,upsert:false});if(uploadError)throw uploadError;const{data,error}=await supabase.from("studynest_resources").insert({uploader_id:req.user.id,title,subject,category,original_name:req.file.originalname,storage_path:storagePath,mime_type:req.file.mimetype,size:req.file.size}).select("id").single();if(error){await supabase.storage.from("studynest-files").remove([storagePath]);throw error}res.status(201).json(data)});
